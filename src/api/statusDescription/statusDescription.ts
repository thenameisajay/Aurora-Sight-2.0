...
import type {
    StatusDescription,
    XmlStatus,
} from '@/types/interfaces/statusDescriptions';

const URL = 'https://aurorawatch-api.lancs.ac.uk/0.2/status-descriptions.xml';

export async function statusDescription(): Promise<StatusDescription[]> {
    try {
        const response = await axios.get(URL);
        const xmlData: string = response.data.toString(); // Convert xmlData to a string

        const parsedData = await new Promise<StatusDescription[]>(
            (resolve, reject) => {
                xml2js.parseString(xmlData, (err, result) => {
                    ...
                    const transformedData: StatusDescription[] = status.map(
                        (item: XmlStatus) => {
                            return {
                                id: item.$.id,
                                color: item.color[0],
